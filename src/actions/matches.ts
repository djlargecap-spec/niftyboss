...
export async function fetchPlayingXI(matchId: string, cricapiMatchId: string) {
  ...
  const pointsResult = await getCricketProvider().fetchMatchPoints(cricapiMatchId)
...
export async function fetchMatchScorecard(
  ...
  const innings = await getCricketProvider().fetchScorecard(cricapiMatchId)
...
export async function previewSeriesImport(seriesId: string): Promise<{
  ...
  const seasonFixtures = await getCricketProvider().fetchSeasonFixtures()
  ...
  for (const [smId, shortName] of Object.entries(getCricketProvider().getTeamMap())) {
  ...
  for (const fixture of seasonFixtures) {
    ...
    const homeCode = getCricketProvider().getTeamMap()[fixture.localteam_id] ?? "?"
    const awayCode = getCricketProvider().getTeamMap()[fixture.visitorteam_id] ?? "?"
    ...
    const dbHomeId = smTeamToDbTeam.get(Number(fixture.localteam_id))
    const dbAwayId = smTeamToDbTeam.get(Number(fixture.visitorteam_id))
...
export async function testMatchPoints(matchId: string): Promise<{ data?: unknown; error?: string }> {
  ...
  const data = await getCricketProvider().fetchMatchPoints(match.cricapi_match_id)
  return { data }
}
